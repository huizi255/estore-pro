...
<!--文件尾开始-->
        ...
        <c:forEach var="p" items="${shoppingCar.lines}">       
		<tr>
			<td class=tablebody2 valign=middle align=center width="">${p.key}</td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="productDetail.jsp?productid=2" target="_blank">${p.value.book.name}</a></td>
			<td class=tablebody2 valign=middle align=center width="">价格：${p.value.book.price}</td>
			<td class=tablebody1 valign=middle align=center width="">数量：${p.value.num}</td>
			<td class=tablebody2 valign=middle align=left width="">小计：${p.value.num*p.value.book.price}</td>
		</tr>
		</c:forEach>
