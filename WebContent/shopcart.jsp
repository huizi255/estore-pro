...
<!--文件体开始-->
		...
		<tr>
			...
			<input type="hidden" name="productid" value="${s.key}">
			<input type="hidden" name="number" value="${s.value.num}">
			<td class=tablebody2 valign=middle align=center width="">${s.key}</td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="productDetail.jsp?productid=2"></a>${s.value.book.name}</td>
			<td class=tablebody2 valign=middle align=center width="">${s.value.book.price}</td>
			<td class=tablebody1 valign=middle align=center width=""><input type="text" name="num" value="${s.value.num}" size="4" onblur="javascript:if(this.value<1){alert('对不起，产品数量不能小于 1 ');this.focus();}else{number.value=this.value;}"/></td>
			<td class=tablebody2 valign=middle align=left width="">&nbsp;&nbsp;${s.value.num*s.value.book.price}</td>
			<td class=tablebody1 valign=middle align=center width="">
			<input type="button" value="取消" onclick="javascript:window.location='removeProduct?productid=${s.key}';"> <input type="submit" value="保存修改"></td>
			</form>
		</tr>
		</c:forEach>
		...
		<tr>
			<td class=tablebody2 valign=middle align=center colspan="6"><input type="button" value="提交订单" onclick="javascript:window.location='confirmOrder';"> <input type="button" value="继续购物" onclick="javascript:window.location='index.jsp';"> <input type="button" value="清空购物车" onclick="javascript:window.location='removeAllProduct';"></td>
		</tr>
                </table><br>
<!--文件尾开始-->
